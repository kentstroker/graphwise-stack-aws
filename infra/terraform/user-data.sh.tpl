...
# ---------------------------------------------------------------------------
# Drop a per-deployment Helm overlay holding ALL operator-editable secrets.
# scripts/reset-helm.sh auto-includes this file via `-f` if present.
#
# Why this file exists rather than editing charts/graphwise-stack/values.yaml
# directly: that file is git-tracked. Editing it on the EC2 means every
# `git pull` is a merge conflict against your real credentials. This overlay
# is gitignored / never tracked / EC2-local -- pull is always a clean
# fast-forward.
#
# Three blocks:
#
#   awsCredentials   -- REQUIRED for full deploy. Bedrock IAM user from
#                       SETUP §4b. Without it: graphrag-components returns
#                       AccessDeniedException on every prompt; chatbot
#                       broken.
#
#   n8nLicense       -- REQUIRED for full deploy. n8n Enterprise key from
#                       Graphwise. Without it: graphrag-workflows pod
#                       refuses to start (n8n license-check fails on boot).
#
#   n8nEncryption    -- AUTO-GENERATED by Terraform random_id (DO NOT EDIT).
#                       n8n encrypts every stored credential with this key
#                       once the pod boots; rotating it makes every saved
#                       n8n connection unreadable forever.
#
# For umbrella-only deploys (reset-helm.sh --skip-graphrag), awsCredentials
# and n8nLicense can stay as placeholders -- GraphRAG isn't installed.
# Required only when running a full deploy.
# ---------------------------------------------------------------------------
...
graphrag-secrets:
  ...
  # SETUP §4b walks creating the graphrag-bedrock IAM user.
  # Region must match the IAM policy ARN region and be Bedrock-enabled
  # (us-east-1 / us-west-2 cover cohere.embed-english-v3 reliably).
  awsCredentials:
    region: "us-west-2"
    accessKeyId: ""           # FILL IN: AKIA...
    secretAccessKey: ""       # FILL IN: matching secret

  # n8n Enterprise license activation key (issued by Graphwise).
  n8nLicense:
    activationKey: ""         # FILL IN

  # AUTO-GENERATED by Terraform. Do not change after first n8n boot.
  n8nEncryption:
