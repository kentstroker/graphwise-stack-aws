...
2. **Bedrock user** — `graphrag-bedrock` → API access only → inline policy `bedrock-graphwise-invoke` granting `bedrock:InvokeModel` + `bedrock:InvokeModelWithResponseStream` on the Cohere embed ARN + Llama foundation-model ARN in each routed region (us-east-1 / us-east-2 / us-west-2) + the Llama inference-profile ARN (`arn:aws:bedrock:<region>:<account-id>:inference-profile/us.meta.llama3-3-70b-instruct-v1:0`) → create access key → save. Full JSON in [SETUP §4b](SETUP.md#4b-create-the-bedrock-iam-user). Default LLM is Llama 3.3 70B (no per-model approval); Anthropic Claude works too but needs a one-time use-case form in the Bedrock console.
...
The chart wires PoolParty 10.2's LLM backend at deploy time (Bedrock + Llama 3.3 70B Instruct via cross-region inference profile by default — see `poolparty.llm.*` in `charts/poolparty/values.yaml` and the override in `charts/graphwise-stack/values.yaml`). That gets the JVM properties set and AWS creds mounted, but the feature itself is gated by a per-deployment **Taxonomy Advisor** instance you have to create inside PoolParty's Semantic Middleware Configurator (SMC). One-time step per deployment.
...
4. In the **Create taxonomy advisor instance** dialog: **Name** = any label (e.g. `bedrock-llama-3-3-70b`); **API Key** = the key Graphwise sent → **Save**.
...
If the IAM policy from §2 wasn't right, the failure surfaces here — `kubectl -n graphwise logs deploy/graphwise-stack-poolparty | grep -iE 'bedrock|accessdenied|inference|resourcenot'` shows the actual Bedrock API error. Common ones: `AccessDeniedException ... is not authorized to perform: bedrock:InvokeModel` on the inference-profile ARN (IAM policy missing the profile ARN — re-check SETUP §4b); `AccessDeniedException` against an `assumed-role/...-ec2-role` principal (AWS creds env vars are empty on the pod — likely missed `-f ~/graphwise-secrets.yaml` on helm upgrade, then needs a pod rollout-restart); `InvalidRequestException ... with on-demand throughput isn't supported` (you swapped to a bare foundation-model ID — must use the inference-profile ID, `us.<model-id>`); `ResourceNotFoundException ... use case details have not been submitted` (Anthropic-only — fill the Bedrock Console use-case form).
