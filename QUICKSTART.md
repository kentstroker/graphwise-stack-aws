...
## 16. Edit `values.yaml` for GraphRAG creds (EC2)

```bash
# EC2 -- never edit this file on your laptop (it's in git with placeholders)
$EDITOR charts/graphwise-stack/values.yaml
```

Find `graphrag-secrets:` and set:
...
Other `change-me-*` values are internal — leave alone.
