...
The file is pre-seeded by Terraform cloud-init with empty placeholders. Fill in the top-level `maven:` block with the Graphwise-issued credentials for `maven.ontotext.com`:
...
`reset-helm.sh` reads these and creates the `graphwise` image-pull Secret in both the `graphwise` and `graphrag` namespaces. Without them, graphrag pods (chatbot, conversation, components, workflows) will `ImagePullBackOff` — the umbrella still installs cleanly, so you can run `reset-helm.sh --skip-graphrag` now and add the creds + re-run later.
...
Details: [DEPLOY §3](DEPLOY.md#3-connect-and-prepare-creds).
...
## 16. Edit `~/graphwise-secrets.yaml` for GraphRAG creds (EC2)
...
The cloud-init-seeded file already has every block laid out — fill in the empty `""` values. The `maven:` block was set in §12; this step fills in the remaining `graphrag-secrets` blocks (leave `n8nEncryption.key` alone — auto-generated by Terraform):

```yaml
maven:
  user:               "<your-graphwise-maven-user>"     # done in §12
  pass:               "<your-graphwise-maven-pass>"     # done in §12
...
Details: [DEPLOY §7](DEPLOY.md#7-configure-graphrag-runtime-credentials).
