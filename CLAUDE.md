...
- *(Resolved 2026-05-28 via operator-supplied platform-independent zip)* **Refine `ontotext/refine:1.2.2` is amd64-only on Docker Hub** (`manifest.v2`, no manifest-list). On Graviton (arm64) the vendor's image CrashLoopBackOffs with `exec /opt/ontorefine/dist/bin/ontorefine: exec format error`. Workaround: Graphwise also ships a **platform-independent ZIP** (Java only, 180 JARs, no native binaries — `refine/ontorefine-*/bin/ontorefine` is a bash launcher that `exec`s java). Operators drop the extracted dist under `refine/ontorefine-1.2.1/` (gitignored, treated like a license file); `scripts/build-refine-image.sh` wraps it in `eclipse-temurin:11-jre` (multi-arch, picks up arm64 natively) and `kind load`s it as `graphwise-refine:local`. `cluster-bootstrap.sh` auto-runs the build when the dist is present and skips silently when not; `scripts/render-values.sh` auto-emits `addons.refine.enabled: true` + the local-image override in the per-deploy overlay when the dist is present. Chart defaults (`refine.enabled: false` in both `charts/graphwise-stack/values.yaml` and `charts/addons/values.yaml`) keep vanilla destroy/apply deploys green for operators who don't need Refine. Remove this entry once Graphwise publishes a multi-arch tag of `ontotext/refine` -- at that point flip defaults to `enabled: true`, repository back to `ontotext/refine`, and delete the dist-detection plumbing.
