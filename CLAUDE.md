...
- **JVM clients** (PoolParty → Keycloak `uma2-configuration`, graphrag-conversation → Keycloak JWKS) have no override mechanism at all. The TLS handshake fails, PoolParty hangs forever in startup-probe loops, the stack doesn't come up.
...
So the chart is wired for `letsencrypt-prod` everywhere, with no operator-facing toggle.

**Rate-limit math.** LE prod limits, by bucket:

| Limit | Window | Scope |
|---|---|---|
| 5 duplicate certs per identifier | 168h | Exact FQDN. EIP doesn't matter — LE only sees the hostname. |
| 50 certs per registered domain | 168h | Per Public Suffix List entry. With ~15 hostnames per deploy, that's ~3 fresh deploys/week across all subdomains of `semantic-proof.com`. |
| 300 New Orders per ACME account | 3h | Usually fine. |
| 5 Failed Validations per account+hostname | 1h | Separate, fast recovery. |

Practical guideline: 5 fresh `reset-helm.sh` cycles per week per exact subdomain before the per-identifier cap; ~3 cycles per week across the whole `semantic-proof.com` zone before the per-domain cap. Mitigations:

1. **Prefer `helm upgrade` over `reset-helm.sh`** — upgrade in place doesn't reissue certs. Use `reset-helm.sh` only when you actually need to wipe PVCs.
2. **Rotate subdomain when iterating fast** — `stroker` → `kent` → `demo`. New per-identifier bucket each time, but you still share the per-registered-domain cap.
3. **Rotate registered domain at the cap** — change `base_domain` in `terraform.tfvars` to a separate apex. Brand-new buckets across all limits.
4. **Long-term escape hatch** — if LE proves too restrictive for the iteration cadence, switch to AWS Certificate Manager via Route 53 + DNS-01 challenge (no per-domain rate limits the same way; ACM is free for AWS-served services). See [ACM migration guide TBD] when needed. For now, prod LE within the rate limits is sufficient.
