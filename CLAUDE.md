...
- **PoolParty Keycloak SPI + theme not loaded in stock Keycloak.** The chart extracts only the realm JSON from `ontotext/poolparty-keycloak` (via `scripts/extract-poolparty-realm.sh`), but the realm references a `poolparty-default-settings-provider` event listener and a `poolparty` login theme that ship as JARs *inside that same vendor image*. Stock Keycloak (Keycloak Operator) has neither on its classpath. Visible as hundreds of `KC-SERVICES0083: Event listener 'poolparty-default-settings-provider' registered, but provider not found` + `Failed to find LOGIN theme poolparty` warnings in `kubectl -n keycloak logs graphwise-keycloak-0`. **Operational impact:** the seeded `superadmin` works (chart startup also seeds its PoolParty-side `skosView` row directly, independent of the SPI), but ANY user created post-deploy has no `skosView` row in PoolParty's DB → first login 500s with `FrontendException: The user setting configuration for [skosView] is missing!` / `error.user.missing.claim`, browser shows INTERNAL ERROR. **Confirmed 2026-05-28**: both the Keycloak admin console AND PoolParty's own user-administration UI fail identically — PoolParty's UI is a thin wrapper around Keycloak's admin REST API, so both trigger the same `CREATE_USER` event and depend on the same missing SPI. Realm-import JSON seeding is suspected to also fail (the chart's `superadmin` seed is a startup-path special case, not a general mechanism) — unverified. **Workaround:** stick with `superadmin` only. **Proper fix (TBD):** build a custom Keycloak image that copies the SPI + theme JARs out of `ontotext/poolparty-keycloak` into `/opt/keycloak/providers/` and `/opt/keycloak/themes/`, then point the operator's CR at it. Estimate: half a day; no registry needed (KIND `docker load`).
