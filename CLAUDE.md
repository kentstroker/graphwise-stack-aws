...
- Helper scripts under `scripts/`: `cluster-bootstrap.sh`, `cluster-resume.sh`, `cluster-stop.sh`, `reset-helm.sh`, `render-values.sh`, `install-licenses.sh`, `extract-poolparty-realm.sh`, `validate-bootstrap.sh`
...
| `scripts/validate-bootstrap.sh` | One-shot post-cluster-bootstrap health check. Read-only; clears screen and walks every operator namespace (cert-manager, ingress-nginx, cnpg-system, monitoring, kubernetes-dashboard, keycloak operator, kube-system metrics-server), the `letsencrypt-prod` ClusterIssuer, the `graphwise` image-pull secret in both consuming namespaces, the `~/dashboard-kubeconfig.yaml` artifact, and a cluster-wide non-Running-pod sweep. Prints color-coded pass/fail per check + an overall verdict. Exit 0 on green, 1 on any failure (so it can gate automation). Run any time after `cluster-bootstrap.sh`. |
