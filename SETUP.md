...
`maven.ontotext.com`. On the EC2 instance these go into the
top-level `maven:` block of `~/graphwise-secrets.yaml` (auto-created
by Terraform cloud-init with empty placeholders, gitignored, never
tracked):
...
The `reset-helm.sh` script reads them and creates the
...
> **Legacy fallback.** Earlier versions of this stack used two
> plain-text files at `~/.ontotext/maven-user` and
> `~/.ontotext/maven-pass`. `reset-helm.sh` still reads them as a
> fallback if the YAML block is empty, but the canonical location
> is now the `maven:` block in `~/graphwise-secrets.yaml` —
...
> creds, Bedrock creds, and n8n license all live in
> `~/graphwise-secrets.yaml`; the n8n encryption key is
> **auto-generated by Terraform** — you don't have to do anything
> for that one. See
> [DEPLOY "Configure GraphRAG runtime credentials"](DEPLOY.md#7-configure-graphrag-runtime-credentials)
> for the full overlay layout.
